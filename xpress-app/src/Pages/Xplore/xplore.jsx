...
const ProductCard = ({ product, navigate, badge }) => (
    ...
    >
        <div className="relative h-56 overflow-hidden bg-gray-50 border-b border-gray-100">
            ...
            />
            {badge && (
                <div className={`absolute top-0 left-0 text-white text-[10px] font-black uppercase tracking-widest px-3 py-1.5 ${
                    badge === 'Featured' ? 'bg-yellow-500 text-black' : badge === 'Hot' ? 'bg-red-600' : 'bg-black'
                }`}>
                    {badge}
                </div>
            )}
